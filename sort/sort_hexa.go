...
func SortFilesByName(absFiles []string) []string {
	...
	for _, f := range absFiles {
		...
		// Optimization
		// ------------
		// strconv.ParseUint(x, 16, 64) panics for hexadecimal WAL file name due to out of range of uint64.
		// Trim the first 8 digits (T section) so that string to uint64 base 16 conversion does not panic. This should
		// be safe to do. Why?
		//
		// The format of wal_file_name in Postgres is a 24-character hexadecimal representation of the LSN.
